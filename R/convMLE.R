#############################################################
# find the MLE of a convex hazard using bisection + support reduction


convexMLE <- function(x, M = 100, GRIDLESS=0, ini.LSE=0, tol=1e-5, tol.SR=1e-8, type=1, max.loop=100, max.inner.loop.SR=250, max.outer.loop.SR=50, range=c(0, max(x)), solve.tol=.Machine$double.eps) 
	{
	...
	sink(file="convexMLE.out", append=TRUE, type="output", split=FALSE)
	cat("negllh","antimode","conv","\n")
	...
	for(i in 1:5){
		temp		 <- 	srMLE(x, m[i], M, GRIDLESS, ini.LSE, tol.SR, max.inner.loop.SR, max.outer.loop.SR, print=0,  solve.tol)
	...
	for(i in 1:max.loop){
		...
		for(i in 1:2){
			temp		 	<- 	srMLE(x, m[2*i], M, GRIDLESS, ini.LSE, tol.SR, max.inner.loop.SR, max.outer.loop.SR, print=0,  solve.tol)
			temp.vec[2*i]	<-	list(temp)		
			phi.vec[2*i] 	<- 	-temp$llh
	...
	return(list(mle=list(supp=res$mle$supp, mix=res$mle$mix), llh=res$llh, antimode=m[j0], conv=(i<max.loop), iter=i))

} # end of convexMLE function
